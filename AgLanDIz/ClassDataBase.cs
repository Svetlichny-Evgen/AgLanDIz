...
namespace AgLanDIz
{
    public class ClassDataBase
    {
        ...
        public int ExecuteNonQuery(string connectionToDBString, string sSql, int where)
        {
            ...
            try
            {
                using (MySqlConnection con = new MySqlConnection(string.Format("server=localhost;user=root;database=ALD_V2;password=0000;", connectionToDBString)))
                {
        ...
        public void Execute<T>(string connectionToDBString, string sSql, ref List<T> listResult)
        {
            ...
            try
            {
                ...
                MySqlConnection con = new MySqlConnection(string.Format("server=localhost;user=root;database=ALD_V2;password=0000", connectionToDBString));
                con.Open();
